    ...
    switch ($action) {
        ...
        case 'get_usage_report':
            ...
            try {
                ...
                switch ($reportType) {
                    case 'registrations':
                        // Check if users table has a registration date column
                        $dateColumnExists = false;
                        $dateColumnName = '';
                        
                        try {
                            // First check for registration_date
                            ...
                            // If neither column exists, add registration_date column
                            if (!$dateColumnExists) {
                                error_log("Adding registration_date column to users table");
                                $alterStmt = $pdo->prepare("ALTER TABLE users ADD COLUMN registration_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP");
                                $alterStmt->execute();
                                
                                // Update existing users with a default date - set to spread over the last month instead of all at once
                                $sql = "SELECT username FROM users WHERE registration_date IS NULL ORDER BY username";
                                $stmt = $pdo->prepare($sql);
                                $stmt->execute();
                                $users = $stmt->fetchAll(PDO::FETCH_ASSOC);
                                
                                // Distribute registrations over the past month to create more interesting data
                                $startDate = date('Y-m-d H:i:s', strtotime('-30 days'));
                                $usersCount = count($users);
                                
                                if ($usersCount > 0) {
                                    $interval = 30 / $usersCount; // days between each registration
                                    
                                    foreach ($users as $index => $user) {
                                        $daysAgo = floor($interval * $index);
                                        $registrationDate = date('Y-m-d H:i:s', strtotime("-$daysAgo days"));
                                        
                                        $updateStmt = $pdo->prepare("UPDATE users SET registration_date = :date WHERE username = :username");
                                        $updateStmt->bindParam(':date', $registrationDate);
                            ...
                            error_log("Error checking/adding registration date column: " . $e->getMessage());
                            echo json_encode([
                                'success' => false, 
                                'message' => 'Error checking registration date columns: ' . $e->getMessage(),
                                'query' => "SHOW COLUMNS FROM users LIKE 'registration_date'",
                        ...
                        $sql = "SELECT DATE({$dateColumnName}) as date, COUNT(*) as count 
                                FROM users 
                                WHERE {$dateColumnName} BETWEEN :start_date AND :end_date 
                                GROUP BY DATE({$dateColumnName}) 
                                ORDER BY date ASC";
                        $stmt = $pdo->prepare($sql);
                        ...
                        $sql = "SELECT 
                                (SELECT COUNT(*) FROM users WHERE {$dateColumnName} BETWEEN :start_date AND :end_date) as current_period,
                                (SELECT COUNT(*) FROM users WHERE {$dateColumnName} BETWEEN :prev_period_start AND :prev_period_end) as previous_period";
                        $stmt = $pdo->prepare($sql);
                        $stmt->bindParam(':start_date', $startDate);
                        $stmt->bindParam(':end_date', $endDate);
                        ...
                        $response['title'] = "User Registrations (" . ($dateRange === 'all' ? 'All Time' : "Last $dateRange days") . ")";
                        break;
